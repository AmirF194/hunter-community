...
**排序与限高由前端做(2026-09-12),后端给什么顺序都行。** 前端的规则是:
`score` 降序 → 没有 `score` 的 → `blocked` 的,同组内保持后端给的顺序。
页面只露 **5 张卡片**,其余在列表内滚动(几十只时不限高,右栏会比左栏长出一大截、
页面左边整片空白)。两个后果:
- **`score` 缺失不会被当成 0 分**,只是排到有分的后面 —— 不确定的分数别硬填一个数。
- **谁排前 5 就决定了用户先看到谁**,所以 `score` 的口径要跨标的可比;
  不可比的话就别给这个字段(给了反而误导),让前端按后端顺序排。

**凑不满 5 条时后端补 RS 最强的(2026-09-12)。** 补位项带 `filler: true`,
并给 `rs_pct`(在同一天有精确 RS Raw 的全部美股里的百分位)、`rs_raw_pct`(近一年加权涨幅):

```jsonc
{ "symbol": "MGRT", "filler": true, "price": 9.9, "rs_pct": 100.0, "rs_raw_pct": 1407.8,
  "score": null, "rule_id": null, "progress_pct": null,
  "gap": "不是今天的候选 —— 它没过任何一条买入规则。近一年加权涨幅 +1408%,在 3696 只有完整一年日线的美股里排第 1" }
```

同时 `watchlist` 顶层给 `matched`(真候选数)与 `filled`(补了几只),**前端分开报数** ——
合成一个"5 只"会让人以为今天有 5 个候选。

补位项是 RS 排名结果,**不是候选**:它没过任何一条买入规则。所以
`score` / `progress_pct` / `rule_id` 必须是 `null`,前端也不会给它画进度条和"距 R-xx 触发" ——
让它长得像候选,就是用排版编造了一个不存在的结论。口径是
`rs_line_stat.rs_raw_exact`(0.4·ROC63 + 0.2·ROC126 + 0.2·ROC189 + 0.2·ROC252,需要 252 根以上日线),
排序时它排在被否决的项之后,也就是整个列表的最末。

> 已知取舍:RS 排名最高的常常是仙股 / 妖股(实测第一名近一年 +1408%)。
> 现在**不加流动性或价格门槛**,因为加了就不再是"RS 排名最高"了。
> 要改成"RS 最高且日均成交额 ≥ X"是一行 SQL 的事,但那是另一个口径,得先说清楚再改。

### 3.10 `trades` · 今日操作报告(**前端 2026-09-12 起不再渲染**)

> 这张卡片撤掉了 —— 历史交易记录(3.10-2)覆盖了它,而且多给了已平仓的净损益与回报。
> 字段还在返回,前端不再读。它独有的两样东西已经搬进 `history.items[].legs[]`:
> `rationale`(为什么买/卖)和 `followup`(卖出后 T+5 的事后跟踪),
> 在信号列 hover 出来。搬过去的原因很直白:卡片没了不等于信息该没。
...
### 3.10-2 `history` · 历史交易记录(2026-09-12 新增)

```jsonc
{
  "fee_note": "模拟盘按收盘价成交,不计手续费与滑点",
  "items": [
    { "no": 19, "symbol": "ARMK", "name": "ARMK", "side": "long",
      "entry_date": "2026-08-11", "exit_date": "2026-08-25",
      "shares": 232, "amount": 14134.64,          // 买入总股数 / 买入总金额
      ...
      "legs": [
        ...
        { "kind": "entry", "date": "2026-08-13", "rule_id": "R-16", "rule_name": "倒三角加仓",
          "price": 61.11, "shares": 66 },
        { "kind": "exit",  "date": "2026-08-25", "rule_id": "R-15", "rule_name": "10 日不涨清仓",
          "price": 59.08, "shares": 232, "pnl_abs": -263.36, "pnl_pct": -1.86,
          "rationale": "持有 10 个交易日仍未涨过 1R,按 R-15 清仓…",   // 为什么卖(hover 出来)
          "followup": "卖出后 T+5 该股再跌 3.2% —— 这次是躲过了" }     // 事后跟踪,只有卖出腿有
      ] }
  ]
}
```

**一条记录 = 一个持仓周期,不是一笔成交。** 倒三角加仓会让一只票有 2~3 次买入、
减半再清仓又是 2 次卖出;按成交平铺出来是 5 行互不相干的记录,看不出"这一笔最后是赚是亏"。
所以按 `(code, entry_date)` 聚合,每次买卖是一条 `leg`,前端用 rowspan 把编号 / 大小 /
净损益 / 回报跨整组显示。

三条硬约束:

- **只给已经平掉的周期**(买入股数 = 卖出股数)。没平完的最终盈亏还没发生,
  写进"历史记录"就是提前写结论 —— 它在 `holdings` 里。
- **回报的分母是这一笔的买入总额**,不是第一笔的成本。加过仓还用第一笔当分母会把回报算大。
- **笔数和 `overview.trades_total` 口径不同**:总览按每次卖出计数(减半再清仓 = 2 笔),
  这里按持仓周期计数(= 1 笔)。实测 88 笔成交 → 19 个周期,而总览显示 47 笔。
  前端必须把这个差异写在卡片里,否则用户会以为少了一半记录。

**`scan_dates` 给悬停日K 画蓝线用(2026-09-12)。** 来自 `agent_watch` 全表
(一天一行、items 是 `[代码, 名称, 评分]`,174 天共 41 kB,全读进来建映射比按代码查 JSONB 便宜)。
给的是这只票**全部**的命中日,不只是这一笔对应的那几天 —— 用户要看的是
「扫描什么时候盯上它、盯了多久、第几天才买」,只给这一笔前后几天就看不出等待过程。
...
每笔最低 0.35 美元,最高为成交金额的 1%,**上限压过下限**(一笔 10 美元的成交收 0.1,不是 0.35)。
档位看的是「当月在这笔之前已成交多少股」,不是当笔大小;跨月重新从首档算。

> 用户只给了首档(0.0035)和末档(0.0005)与上下限,中间三档按 IBKR Tiered(美股)的公开档位补全 ——
> 首末两档都对得上,出处就是它。**中间档位是假设**,换券商只改 `commission.TIERS` 一处。

`history` 顶层同时给 `fee_total` / `pnl_gross_total` / `pnl_net_total`,前端要在脚注里写出
"扣费前 → 扣费后"的差额。实测 19 笔 88 条腿共 48.88 美元,吃掉毛利的 5.1%。

**手续费只进这张表,不进净值曲线。** `overview` / `nav` / 胜率仍是引擎的零费用口径:
引擎按收盘价成交、不扣现金。要让净值曲线也扣费,得改 `agent_vcp.py` 的建仓与加仓
(现金变少会买不起同样股数 → 仓位变 → 整条曲线和每一笔成交都会变),并重跑全部历史。
