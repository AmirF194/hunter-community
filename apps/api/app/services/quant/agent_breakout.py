...
- P-01 市场环境:标普 500 收盘 > 50 日均线 > 200 日均线;不满足不开新仓(持仓按 P-15 退出)。
- P-02 价格与流动性:收盘 > 20 美元,30 日均量 > 20 万股。
- P-03 趋势:收盘 > 50 日 > 150 日 > 200 日均线;收盘 > 近 252 日最高价 × 0.80;RS ≥ 80。
- P-04 整理形态:3 个月振幅 12%~35%、1 个月振幅 3%~12%、1 月振幅 ≤ 3 月振幅 × 0.55、
        5 日振幅 ≤ 1 月振幅 × 0.65、近 21 日最低 > 近 63 日最低 × 1.01(振幅 = (最高 − 最低)÷ 最高,窗口含今天)。
- P-05 枢轴与缩量:枢轴 = **前一天为止**的近 21 日最高价(脚本 `highest(high, 21)[1]`);距枢轴 −4% ~ +5%;
        10 日均量 < 50 日均量 × 0.9。
- P-06 突破触发:收盘 > 枢轴,收盘 < 枢轴 × 1.05(不追高超过 5%),当天成交量 > 50 日均量 × 1.5。
...
- P-08 加仓:① 收盘 > 进场价 × 1.02、量 > 20 日均量 × 1.2、收盘 > EMA8、市场向上 → 加完整仓位的 30%;
        ② 收盘 > 进场价 × 1.05、量 > 20 日均量 × 1.1、收盘 > EMA21、市场向上 → 再加 20%。按顺序,每次一只票一天最多加一次。
...
5. 均量窗口含今天(和扫描源 average_volume_Nd_calc 同口径);`[1]` 的两处(枢轴、Base 低点)不含今天。
...
ENTRY_RULE = "P-06"
ADD_RULE = "P-08"
WATCH_POOL_DAYS = 1                  # 突破是当天的事;筛选条件由引擎在当天重算,池子只要当天的
...
"""
EXEC_NOTE = "纸上交易 · 日线收盘价成交(没有开盘价,阳线条件未实现)"

RULES = [
    {"id": "P-01", "kind": "risk", "condition": "市场环境:标普 500 收盘 > 50 日均线 > 200 日均线;不满足不开新仓"},
    {"id": "P-02", "kind": "buy", "condition": "价格与流动性:收盘 > $20,30 日均量 > 20 万股"},
    {"id": "P-03", "kind": "buy", "condition": "趋势:收盘 > 50 日 > 150 日 > 200 日均线;收盘 > 近 252 日最高 × 0.80;RS ≥ 80"},
    {"id": "P-04", "kind": "buy", "condition": "整理形态:3 月振幅 12%~35%、1 月振幅 3%~12%、1 月 ≤ 3 月 × 0.55、5 日 ≤ 1 月 × 0.65、21 日最低 > 63 日最低 × 1.01"},
    {"id": "P-05", "kind": "buy", "condition": "枢轴与缩量:枢轴 = 前一天为止近 21 日最高;距枢轴 −4% ~ +5%;10 日均量 < 50 日均量 × 0.9"},
    {"id": "P-06", "kind": "buy", "condition": "突破触发:收盘 > 枢轴、收盘 < 枢轴 × 1.05、成交量 > 50 日均量 × 1.5(阳线条件没做:日线没有开盘价)"},
...
def summary(p: dict = PARAMS) -> str:
    return ("Patrick Walker 风格突破:标普在 50 日 > 200 日之上才做;筛出均线多头、离一年高点 20% 以内、RS ≥ 80、"
            "3 个月 → 1 个月 → 5 天振幅逐级收紧、低点抬高、量能干燥的票,收盘放量(> 1.5 倍 50 日均量)站上前 21 日最高、"
...
def indicators(bars: list[tuple], p: dict = PARAMS, bench: dict | None = None) -> dict | None:
    """bars = [(d, c, h, l, v)] 升序,最后一根是今天。持仓管理要的字段只要 60 根;
    筛选字段要 252 根且窗口里没有缺值,算不出时 screen = None(这只票今天不买,不猜)。"""
    n = len(bars)
    if n < MIN_BARS:
        return None
    c = [b[1] for b in bars]
...
def entry_checks(ind: dict, p: dict = PARAMS, score=None, market: dict | None = None) -> list[dict]:
    """→ [{rule, ok, text}] 按 P-01 ~ P-06。每条的文字写实际数字,观察列表和买入理由共用。"""
    px, sc = ind["close"], ind.get("screen")
    out = []
    mk_ok = bool(market and market.get("ok"))
    out.append({"rule": "P-01", "ok": mk_ok, "text": market["text"] if market else "没有基准日线"})
    if sc is None:
        t = "日线不足 252 根或窗口里缺高低量,筛选条件算不出"
        for r in ("P-02", "P-03", "P-04", "P-05"):
            out.append({"rule": r, "ok": False, "text": t})
    else:
        ok2 = px > p["price_min"] and sc["av30"] is not None and sc["av30"] > p["liq_min"]
        out.append({"rule": "P-02", "ok": ok2,
                    "text": f"收盘 ${px:.2f},30 日均量 {sc['av30'] / 1e4:.0f} 万股" + ("" if ok2 else f"(要 > ${p['price_min']:.0f} 且 > {p['liq_min'] / 1e4:.0f} 万)")})
        s50, s150, s200 = ind["sma50"], sc["sma150"], sc["sma200"]
        ...
        if not near:
            bad.append(f"离一年最高 ${sc['hi252']:.2f} 超过 {(1 - p['near_high']) * 100:.0f}%")
        if not rs_ok:
            bad.append(f"RS {score:.0f} < {p['rs_min']}" if score is not None else "RS 缺")
        out.append({"rule": "P-03", "ok": not bad, "text": "趋势通过" if not bad else ";".join(bad)})
        ...
        txt = (f"振幅 3 月 {r3 * 100:.1f}% → 1 月 {r1 * 100:.1f}% → 5 日 {r5 * 100:.1f}%" if None not in (r3, r1, r5) else "振幅算不出")
        out.append({"rule": "P-04", "ok": not bad, "text": txt + ("" if not bad else ";" + ";".join(bad))})
        pv = ind["pivot"]
        dist = (px - pv) / pv * 100
        vdry = ind["av10"] is not None and ind["av50"] is not None and ind["av10"] < ind["av50"] * p["vdry"]
        ok5 = p["pivot_lo"] <= dist <= p["pivot_hi"] and vdry
        out.append({"rule": "P-05", "ok": ok5,
                    "text": f"距枢轴 ${pv:.2f} {dist:+.1f}%,10 日均量 ÷ 50 日均量 {ind['av10'] / ind['av50']:.2f}"
    ...
    out.append({"rule": "P-06", "ok": brk and not_ext and surge, "text": t6})
    return out
