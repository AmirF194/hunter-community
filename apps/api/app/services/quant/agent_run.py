...
_V1 = {
    ...
    "breakout": ("突破买入 v2 —— 标普上升趋势 + 前一天收盘过 Clean Simple Base 筛选(RS ≥ 70)+ 放量突破浪高点下方密集成交区上沿 + 两次加仓 + 部分止盈 + 三种止损",
                 "用户 2026-09-13 给的 Patrick Walker 风格完整脚本逐条移植;同日用户把枢轴改成密集成交区、RS 降到 70"
                 "(v1 枢轴 = 前 21 日最高、RS ≥ 80:一年 -4.72%、21 笔);没有开盘价,阳线条件没做"),
}
