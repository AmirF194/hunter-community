...
- 候选池 = 筛选器时间回溯跑 POOL_SCRIPT(宽松预筛:涨幅门槛按老 ST 的 4.8%、不涨停门槛按 20%),
...
POOL = "limitup"
POOL_LIMIT = 500
POOL_LABEL = "涨停后强势整理预筛池(T-3 涨幅 ≥ 4.8% · 之后三天每天涨幅 < 20% · 三天收盘都高于 T-3 收盘;板块涨停门槛由引擎精确判)"
POOL_SCRIPT = """# ===== 涨停后强势整理 · 宽松预筛(小鹿 · A 股研究线)=====
# 脚本里分不出板块,这里按最宽的门槛圈池:涨停按老 ST 的 4.8%、不涨停按创业板的 20%;
# 主板 10% / 创业板科创板 20% / 主板 ST(2025-07-07 前 5%)由引擎按代码、名称、日期精确判
def up_t3  = (close[3] - close[4]) / close[4] >= 0.048;
