...
**点官方示例 = 直接换成那套条件(同日用户要求)**:原来点示例是把脚本塞进生成框再 `generate()`,受「追加 / 替换」开关影响,
追加模式下拿当前条件当上下文编译 —— 上面那次事故就是这条路径触发的。用户原话「官方预留的就直接记录筛选器就行,不要再生成一次」。
现在 `loadOfficialPreset` 和加载「我的扫描策略」一样一律替换:不进生成框、不看追加开关、失败时市场退回、条件不动,toast 说替换了几条。
render_check「官方示例」7 条盯着(含源码断言:点示例的分支里不许再出现 `generate()`)。
