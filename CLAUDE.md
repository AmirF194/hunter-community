...
6. **美股 `rs_daily` 不按保留期删除(2026-09-15 用户:「已有的美股日线数据不要删除」)**。原来每晚 `run` 结尾删 900 天以前的行
   (最初 480 天,理由是 252 天 ROC / 21 日均线用不到更早的;09-12 为小鹿回测放到 900),美股当时因此只剩 2024-04 起。
   现在只删 A 股 / 港股,**别把美股加回去** —— 三年回测要 2022 年起的日线,`fetch-older` 补一轮 70 分钟。
   代价:美股表只增不减,`compute_market` / `screen_asof._load_store` 读整窗会随年数变慢变大,真慢了再按读取窗口限根数,不要靠删数据。
