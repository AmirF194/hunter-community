...
### 5. A 股 / 港股的名称、板块是英文,结果行统一过 `screen_cn` 换中文(A 股 2026-09-17、港股 09-19 用户要求)
...
- **港股同样换中文(2026-09-19 用户要求)**:入口改成 `screen_cn.localize_pick(p, md.key)`(`localize_a_pick` 保留为老接口),
  港股按 **5 位代码**查 `data/hk_names_baseline.json`(akshare `stock_hk_spot` 新浪源生成,2803 只,当天港股扫描池 2634 只覆盖 2618 只;
  没覆盖的 16 只多是信托 / 新上市,保留英文),后台每 24 小时同样用 `stock_hk_spot` 补(线上 23 秒;**东财 `stock_hk_spot_em` 本机和 fin-r1 都连不上,别换过去**)。
  板块与 A 股同一张对照表。美股不动。清单要重新生成时照 `test_screen_cn.py` 断言的格式(`items: [{code 5 位, name}]`)。用例 `tests/test_screen_cn.py`。
...
**第 6 轮(2026-09-17)· 界面上看得到的字段,生成时必须认得**:用户在「可用字段」点了 MACD.hist,生成报「本地识别没看懂」——
「自己的产品不认识自己的库」。**不是词表没同步**,是三类问题叠在一起,拿美股 3809 个字段名按写法类别全量探针查出来的:
1. **只写字段名、没写怎么比**(列表里 1096 个字段全中):字段是认得的,缺的是比较。现在抛 `screen_kw.MissingComparison`,
   报错点名字段和中文名、给写法示例;`parse_script` 对它**不转 NeedsAI、不给 AI 按钮** —— 阈值只能用户定,AI 补的数字就是编的。
2. **列表显示的中文名打进来认不出**(576 个中文名里 380 个):`_label_aliases` 把 `field_label_cn` 收进词汇。
   同名多字段不收、报错点名是哪几个(历史最低 = Low.All / all_time_low);两字短名只在句首且紧跟比较词才认;和词表重名以词表为准。
   「倍数」拒绝规则改成只看字段名**之外**的文字(8 个「…保障倍数(TTM)」原来全被当成倍数句型拒掉)。
3. **列出来却写不进脚本**(带 `-` `+` 或数字开头,21 个:ADX±DI*、daily-bar.time、24h_vol_to_market_cap …):
   词法根本切不成一个词。`screen_source.listable_fields` 按 `screen_dsl.is_writable_name` 过滤,**列表里出现的必须写得进脚本**;
   两处共用这一个判定,改词法的 ident 规则要一起改。界面上写死的「3777 个」一并去掉(能用的只有 1096 个)。
**以后加字段 / 改中文名 / 改词法,都按类别跑一遍全量探针**:对 `listable_fields` 逐个试「裸名 → 缺比较」「名 > 0」「名大于0」「中文名大于0」,
四类都要清零(同名多字段那几个按设计报点名错误);再拿 test_screen_kw 全部句子在**真实字段全集**下比新旧版本,差异只能是新加的用例。
用例 `test_screen_kw.py` 本轮 +16 条(含 `SHOULD_MISS_CMP` 组,检查异常类型,防止缺比较又退回成给 AI 按钮的普通错误)。

**第 7 轮(2026-09-18)· 金额当成股数、区间当成当日**:用户本地实测「股价站上50日均线,成交额大于2000万,近一个月涨幅超过10%」
产出 `volume > 20000000` 与 `change > 10`,两条都静默错。根因:词表里写死了 `"成交额": "volume"`;「近一个月」这类区间词不认,
「涨幅」被单独截出来 = 当日涨跌幅。按类别查又挖出两条同类:「跌幅超过10%」→ `change > 10`(方向反了)、「换手率」→ 量比。现在:
1. **成交额走 `_AMOUNT_RE`**:当日 → `close * volume`(扫描源 metainfo **没有**当日成交额字段,`Value.Traded` 写进脚本报不认识;
   提示区写明口径);`N日均成交额` → `AvgValue.Traded_{N}d`,**只有 10/30/60/90 天**,其余周期、没说几天、「20日成交额」(合计还是日均)
   一律拒绝并给写法。`_unit` 新增「成交额」类,和成交量 / 价格都不能比。
2. **区间涨幅走 `_PERF_RE`**(区间词紧挨在涨幅 / 跌幅前):近5日 / 近1周 / 1·3·6·12 个月 / 半年 / 1·3·5·10 年 / 年内·今年以来 / 上市以来 → `Perf.*`。
   扫描源按**自然日历滚动**算,近20日 / 近30天 / 近两周 / 本月 **不换算**,拒绝(报错里给能用的写法和 `close / close[20]` 脚本写法)。
   **兜底**:句子认成了 `change`、字段之外还有区间说法(`_PERIOD_HINT_RE`:近 / 最近 / 过去 / 以来 / 数字+周月年…)→ 拒绝,**绝不退回当日**。
3. **跌幅**只在「一个字段 + 一个数」里翻转(`跌幅超过5%` → `change < -5`,写进提示);区间、字段对字段、负数阈值一律拒绝。
4. 「换手率」从词表删掉:扫描源没有换手率字段,认不出比认成量比好。`_CN_NUM_RE` 补了「个月 / 星期」(「近一个月」原来不转数字)。
5. `_clause_to_expr` 的说明先记本句、**认出来才并进 notes** —— 认不出的句子会被对照表补上,留着说明就成了对别的表达式的错误解释。
验证:用例 +42 应识别 / +27 应拒绝;真实字段全集下新旧比对差异只有新用例;全量探针与旧版一致。探针里 16 处**既有**差异与本轮无关,
其中两处是真 bug(第 8 轮已修):`recommendation_under > 0` → `< 0`(英文比较词 under 从字段名里被读出来)、「盘前变动(绝对值)大于0」→ `change_abs > 0`。

**第 8 轮(2026-09-18)· 字段名里的词被读成比较 / 被截成别的字段**:修的就是上面第 7 轮探针留下的两处,按类别是两类:
1. **比较词没有词边界**:`_find_op` 原来对每个比较词裸 `low.find(w)`,英文比较词会从标识符和普通单词内部读出来 ——
   `recommendation_under > 0` → `< 0`、`recommendation_over小于5` → `> 5`(over 比「小于」靠前)、
   `RSI oversold 30` → `RSI > 30`(**超卖读成大于**)、`RSI overbought 70` / `close overshoot 10` / `市盈率undervalued 10` 全都产出条件。
   现在两道一起挡:① 英文比较词按词边界找(`_op_re`,边界含 `_ . |`,和英文字段词同一口径),`_rs_line_expr` 与 `_CMP_WORDS` 共用;
   ② 通用分支改成在 `outside`(字段名挖掉后的文字)上找比较词 —— 中文字段名里夹「高于 / 超过」之类同样不会被当比较。
   **以后加英文比较词直接加进 `_OP_WORDS`,不要在别处另写一份 find**;真实字段里含比较词的目前只有 recommendation_over / _under。
2. **同名多字段中文名里的子串被截走**:「盘前变动(绝对值)」= pre_change_abs / premarket_change_abs,按设计不收,
   但里面的「变动(绝对值)」(change_abs 的中文名)照样被认成 change_abs。现在 `_candidates` 把 `vocab.ambiguous` 的每处出现
   **当作不认识的标识符**(落在它内部的更短候选一律作废),句子认不出后由 translate 报「同时是哪几个字段的中文名」并点名。
   这和 `_SHADOW`(涨跌量比)是同一类:**长词拿不准时,它里面的短词也不能拿出来用**。
用例 +10 应识别 / +8 应拒绝 / 新增 `SHOULD_AMBIG` 组 3 条(检查报错点名全部字段,防止退成「没看懂」或命中别的字段)。
验证:真实字段全集(美 / 港 / A)下 330 句新旧比对差异 15 句,全是本轮新用例;全量探针差异只有这 4 项,其余 14 条 label 失败是既有的按设计行为
(词表优先的中文名如 毛利率 / 市净率,同名多字段如 历史最低 / 市销率 / 盘前变动)。
...
**⚠ 2026-09-17 改口径:复制出来的东西必须能粘回自家生成框。** 用户复制 `def c_price_2 = close > 10;` 粘回生成框报「脚本里没有 plot 语句」——
自家复制的东西自家不认。浏览器里拿 7 个官方示例 + 3 个保存策略共 127 行逐行「复制 → 追加 / 替换」实跑,修前替换模式 77 条条件里 19 条失败
(引用中间定义)、所有片段都缺 plot。现在三处一起管:
1. `copySnippet` **连同间接用到的参数与中间定义一起复制**(参数在前、中间定义其次、都按脚本先后;上面「中间定义不带」作废),片段自给自足;
2. 后端 `screen_dsl.complete_fragment`:没有 plot 的片段按「没被引用的布尔 def + 结尾裸表达式」补 plot(数值定义 / input / rec 不算条件,一条都没有就报清楚);
   漏写的最后一个分号补在代码末尾;
3. 追加模式前端把 `buildScript(true)` 作为 `context` 传给 `/screener/parse`:片段拼在当前脚本后编译,只把片段自己的条件行还给前端;
   和当前脚本**一字不差**的依赖沿用原来的(不多出 `rng3m_2`),条件本身重名照样改名追加,定义不同的同名依赖改名并改写片段内引用。
**改复制按钮 / 生成框 / 脚本语法时,重跑那 127 行的浏览器实测**(思路:对每个示例和保存策略,`reparse` → 每行 `copySnippet` →
`S.mode='append'|'replace'` + `generate()`,断言条件行两种模式都成功、追加后 `buildScript(true)` 仍能解析)。
用例 `tests/test_screen_fragment.py` 25 条 · render_check「编辑框参数值」+4 条。
...
- **「快照字段按今天的值当常量」这条提示不能删**(2026-09-19 补回):5540bce 按用户要求去掉「怎么算 / 求值截至」两条说明时,
  把同一个字符串里的这半句一起删了,`test_screen_series_run.py` d 组从此 36/37。它不是「怎么算」的说明,是口径声明 ——
  市值 / PE / 财务没有历史,整段 K 线都拿今天的值比,不说用户会当成逐日历史值(空的比假的好同一类)。
  现在单独一条,只在「不回溯且脚本用了快照字段」时出,点名字段。以后再删提示,按句拆开看,别整段删。
...
2. **逐字就是字段名 / def 名的一律不动,字段逐字匹配优先**;其余先认自己的 def / input / rec(含追加模式的上下文脚本),再认字段。
   ⚠ 第一版写的是「def 与字段同名不同大小写时引用归 def」,上线当天(09-22)出事:当前条件是「上升趋势」(`def sma50 = Average(close, 50)`),
   追加模式点「VCP 波段收缩」,VCP 里写得完全正确的 `SMA50` 被改成上下文里的 `sma50`,报「不认识 'sma50'」。
   **改这条规则后必须跑「官方示例两两追加」探针**(7 个示例 × 互为上下文 = 42 组 + 单独 7 组,全部解析得过;脚本思路:
   `for a, b in PRESETS: parse_script(b.script, b.market, False, None, None, a.script)`)。
...
**点官方示例 = 直接进条件区,按「追加 / 替换」开关合并(同日用户要求)**:原来点示例是把脚本塞进生成框再 `generate()` ——
上面那次事故就是这条路径(追加模式拿当前条件当上下文编译)触发的。用户原话「官方预留的就直接记录筛选器就行,不要再生成一次」。
现在 `loadOfficialPreset`:**不进生成框、不动生成框里用户正在写的字、不用再点「生成」**;替换模式(或当前没有条件)整套换成示例那套,
追加模式走 `generate(false, preset)` 的追加路径(上下文编译、重名改名、回写失败整份退回),失败时市场退回、条件不动、不给 AI 按钮。
⚠ 第一版一律替换,用户当场纠正「我选的追加,它点击后却是替换」—— **开关是用户的明确选择,别替他决定**。
render_check「官方示例」13 条盯着(含源码断言:点示例的分支里不许直接 `generate(`)。
...
小鹿智能体所有接口 500(`77e49a6` 修)。**往已有 `_DDL` 里追加 `ADD COLUMN` 时,放在那张表 `CREATE` 的后面**,
并在本机 postgres 里建空库执行一遍、再执行一遍验幂等 —— 只在线上库上验,顺序错了也是绿的。
...
- **扫描当日以结果表为准(2026-09-19 用户:「显示命中的蓝色线一定是基于当前筛选器配置命中的,这点原则一直不能变」)**。
  快照类脚本的结果表用扫描源快照求值,蓝线用自家日线回算,扫描当天会对不上:自家日线还没到那天(本地版不拉数时整只 0 天)、
  均线 / 量 / 高低价差一点卡在边界、票不在日线池。本地「放量突破」实测修前 134 只里 40 只扫描当日没蓝线。
  现在前端对结果表里的票传 `in_result: true`,后端把快照所属交易日(`daily-bar.time`,UTC 零点时间戳)一律记为命中,
  回算不一致时在图例**常驻**写明原因(`noteShow`);过去的日子仍按自家日线回算。三条别改坏:
  ① 回溯(as_of)和时间序列脚本不走这条 —— 它们和结果表本来就是同一份日线同一套算法;
  ② **日K 图最后一根一定是蓝的(同日用户追加)**:冷门票 / SPAC 在快照里的最新日K 常停在前一两天(之后没成交),日K 源(`/api/kline`)却补了零成交 / 一字线的一根,按快照日补标就落在倒数第二根。后端补标时回 `scan_pinned` + `scan_note`,前端 `pinLastBar` 读 kcFetch 同一份缓存:最后一根比快照日新 → 补标挪到最后一根(快照日若是回算真命中则保留、最后一根另加);日K 源比快照日还旧(实测 SBNC)→ 不硬标,图例写「日K 图数据只到 X」。补标说明有「算不出」的天也常驻显示;
  ③ 前端 HITS 缓存键带 `runAt`,重新扫描就重新取,不拿上一轮的结论。用例 `test_screen_hits.py` 第 4 组、render_check「扫描当日蓝线」5 条 +「最后一根蓝线」6 条(抠出 pinLastBar 用假日K 真跑)。
...
- **回溯时悬停日K 画到最新收盘,回溯日画竖线(2026-09-16 用户:「使用回溯后的扫描,K 线图的右侧范围也应该是到今天的前一天收盘」)**:`KC.asOf` 不再截断数据,`kcRender` 把它并进 mark(`asOfDay`),`kcMarkSeries` 在那根上画琥珀色虚线并标「回溯日」,图例写明「竖线右侧是扫描当时看不到的走势」。**别把截断改回来** —— 原口径怕露出答案,用户要看回溯之后走成什么样;竖线保住了「扫描站在哪一天」。筛选器的蓝带仍只回算到回溯日往前 250 个交易日,竖线右侧没有蓝带是数据口径。
- **echarts 弹层首开 0 宽必须靠 ResizeObserver,别用同步补量(2026-09-16 实测两次)**:`echarts.init` 那一刻弹层还没排版,量到宽度 0;`kcPlace` 之后**同步**比对 `getWidth()` 与 `clientWidth` 也没用(那一刻容器仍是 0,条件被短路跳过),第一张图整张空白、第二只票才正常。现在 init 后 `kcWatchSize(box)` 挂 ResizeObserver,尺寸一变就 `kcFit()`。排查「图空白但 getOption 有数据」先量 canvas 宽:0 就是这个,不是数据问题。
- **悬停日K 的根数按页面配(2026-09-16 用户:「既然已经有 3 年数据了,K 线图上也支持 3 年,不然看不到前两年的买入点」)**:`KC.limit`(app.js),小鹿看板 `boot` 里设 `KC_LIMIT_3Y`(750 根),筛选器不设、仍是 250。三条别改坏:① **筛选器别跟着改成三年** —— 那边蓝线是「这次运行的脚本」只回算 250 个交易日(`screen_hits.DAYS`),图给三年、蓝线只有最后一年,会像前两年从没命中;要一起改先改 DAYS(每只票耗时同比例上升)。② 缓存键是 `代码@根数`,不能只用代码 —— 两个页面共用 `KC.cache`,串了就会把一年的数据画成三年。③ 750 根铺满约 470px 的画图区一根不到 0.6px,**首次视图由 `kcFocus` 对准买卖标记**(前 45 后 35 根、至少 120 根),没有标记退回最近一年,一年以内的数据返回 null(筛选器照旧全段);拖动 / 缩放后重画仍沿用用户的区间(`kcZoomOf` 优先)。线上实测:CASY(2023-11-17 买入)图上 750 根、默认窗口 121 根覆盖那笔买入。render_check 有 13 条断言。
...
2. **AI 次数只在真调模型的那一刻扣**:`parse_script(..., on_ai=)` 在 `fix_script` / `translate` 之前回调。点「生成」走脚本和本地关键词,零 token 不扣。
   ...
   「调用模型失败」后面的原因由 `screen_nl.model_error_text` 按状态码写中文(402 余额 / 401·403 密钥 / 404 模型名 / 429 / 5xx / 超时 / 连不上,
   认不出的如实写状态码),上游英文原话只进日志(2026-09-19:DeepSeek 余额 0 时报错条原样显示 `Error code: 402 - {'error': …}`)。
   **改这段文案必须保留「调用模型失败」前缀**,路由靠它判断退次数;test_screen_fix 有断言。
...
4. **5 秒两层**:前端扫描返回后倒数 5 秒才放结果(`revealAfter`,只改两处数字不整页重画 —— 整页 render 会弄丢生成框的光标);
   ...
   **秒数可配(2026-09-17 用户:本地自用版扫完立刻显示)**:env `SCREEN_SCAN_GAP_S`(compose 默认 5),`/screener/quota` 回 `scan_gap_s`,
   前端 `revealSeconds()` 按它倒数,0 就不调 `revealAfter`。两条别改坏:① 缺值 / 负数 / 非数字**一律按 5**(前后端都是),不按 0 ——
   配置写错不能悄悄把保护关掉;② 只关「等」,`fetch_rows` 那层上游保护(第 5 条)不跟着这个变量走。线上多人部署保持 5。
   注意时间序列脚本(带 `[1]` / 递归的)读的是自家日线 `rs_daily`,本来就不打上游,等 5 秒对它纯属用户体验成本。
...
8. **突破买入线(`agent_breakout.py`,方向 `breakout`,2026-09-13 用户给的 Patrick Walker 风格完整脚本)。** 四条别改坏的:
   ...
   **P-24 全年只触发 2 次、都是浮盈 > 10% 卖一半(NXT 10-21、ORA 10-30),两只都在财报后涨了约 9%,比 v13 满仓扛少赚 878**。
   ...
   ⑤ **v14 三年结果(2023-09-15 ~ 2026-09-14)**:+6.84%(标普 +71.22%)、回撤 -9.03%、89 笔(赚 32)、每笔 +77、有持仓天数 46%。
   逐年:第 1 年 +2.69%(标普 +26.42%)37 笔每笔 +73;**第 2 年 -4.94%(标普 +16.94%)、回撤 -8.67%、26 笔每笔 -195**;
   第 3 年 +9.44% 与一年方向对得上(26 笔、赚 12;每笔 +354 对 +363 是本金不同、仓位按总资产 20% 算)。筛选告警 / 失败 0 天。
   卖出按规则:P-11 初始止损 49 笔 -17,442 仍是最大拖累;P-15 大盘转弱 26 笔 +10,561 是主要利润来源。
   P-23 三年挡下 55 次;P-24 13 条腿(清仓 9、减半 4)已实现 +5,213,**反事实(拿到财报后第 2 个交易日收盘)合计少赚约 3,700**,
   6 次躲过下跌、7 次卖飞(CHEF +20%、HWM +13.8%、CNM +13.7%)—— 两轮样本都显示 P-24 偏负贡献,但只看到 E+2,不含之后的路径。
   P-23 三年的真实贡献要跑一版 `earn_filter=False` 的三年对照才知道(筛选与指标已落库,重跑只要几分钟)。
   局限:股票池是今天还在上市的票(幸存者偏差);拆股锚点只到 1 年,早年个别票的老段可能被截短。
   耗时:三年回填 82 分钟,回测步骤 4,816 秒几乎全是 2025-09 之前约 500 天的第一次回溯筛选;这些已落库,下一轮复用。
   ⑥ **第 2 年(-4.94%)归因(2026-09-16 用户问「和 2025 年 4 月那波大跌有没有关系」)—— 没关系,恰好相反**:
   4 月整月**空仓**(有持仓天数 0%、0 笔),大跌一分没亏。亏损来自两段:
   (一)**2024-10 ~ 12 震荡磨损**:那段有持仓天数 91%、20 笔进场 -3,638(标普 +4.4%);第 2 年 P-11 初始止损 15 笔 -4,969。
   (二)**错过 2025 年 4~6 月反弹**:2025-02-10 出场后到 07-02 近 5 个月一笔没买,5~9 月有持仓天数只有 12%,标普同期 +17.55%。
   机械原因在 P-01 的「50 日线 > 200 日线」这条腿:3 月 50>200 成立但收盘 21 天 0 天站上 50 日线;4 月两条都不成立;
   **5 / 6 月收盘 21 / 20 天全部站上 50 日线,但死叉未修复、50>200 有 0 天**,标普这两个月 5,604 → 6,198(+10.6%)全程空仓;7 月才两条同时成立。
   同期并不缺候选:5 月 152、6 月 323 个「形态就绪」票天,只差 P-01。要改市场过滤(如换成收盘 > 200 日线、或 200 日线上行 + 收盘 > 50 日线)
   是用户的决定,别自己改;筛选与指标已落库,三年对照重跑只要几分钟。
...
9. **涨停后强势整理线(`agent_limitup.py`,方向 `limitup`,2026-09-17 用户要求)—— 第一条 A 股研究线。** 五条别改坏的:
   (a) **小鹿从此按方向区分市场。** 引擎声明 `MARKET` / `INITIAL_CAPITAL` / `CURRENCY*` / `BENCH_LABEL`,不声明的一律美股 + 10 万美元
   (`agent_opt.market_of / initial_capital / currency`)。`Ctx(market)` 一次只装一个市场的日线;`run_date` 只跑 ctx 那个市场的方向;
   `backfill` 混市场直接报错;`run_latest` 先跑美股,再给每个其他市场各开一个 Ctx 跑那个市场最新的交易日(失败不挡美股)。
   **新加引擎、新加读 `initial_capital` / `美元` / `$` 的地方,一律走这三个函数**,别再写 `av.GUARDS["initial_capital"]` 或写死美元。
   研究台判「追到最新交易日」也按市场分开取 max —— 混在一起的话 A 股比美股早一天,美股线永远判成没追完。
   (b) **口径是立项时用 AskUserQuestion 问出来的**:守住 = 三天收盘**严格高于**涨停日收盘(用户自己写的,不是开盘价)、涨停按板块、
   每个信号独立买 1 万(本金 100 万,一天最多 66 个信号永远不缺钱)、手续费按 A 股实际扣。改之前先问用户。
   (c) **ST 口径立项当天更正过**:给用户的选项写的是「ST 5%」,核对时发现 300044(创业板 *ST)涨 10.29% 被判成涨停。
   现行:创业板 / 科创板含 ST 一律 20%;沪深主板 ST 2025-07-07 起 10%、之前 5%(`limit_of(..., on=日期)`)。ST 按**今天**的简称判,
   历史戴帽摘帽时点拿不到。
   (d) **这条线的手续费从现金里扣**(`commission.a_share_fee`:佣金万 2.5 最低 5 元 + 过户费 0.001% 双边 + 卖出印花税 0.05%),
   所以总览 / 净值是扣费后的;成交表 `pnl_abs` 仍是扣费前,`_trade_rounds(market="a")` 按同一个函数算费用再减 —— 不会重复扣。
   美股线仍是「引擎零费用、只在成交表扣」,两种口径靠 `scope_note` 写明,别统一成一种而不重跑历史。
   (e) 池子脚本是宽松预筛(脚本里分不出板块):T-3 涨幅 ≥ 9.8%、之后每天 < 20%、三天收盘高于 T-3;**板块门槛只在引擎里判**。
   第一次全年回填用 4.8%(为了圈到老规则下主板 ST 的 5% 涨停),4 天命中 548~726 只,**超过筛选器单次 500 只上限被静默截掉**
   (按收盘价降序,低价票没进观察列表)—— 收紧到 9.8% 后清库重跑。`_screen` 现在命中超过上限会打 WARNING,回填完 grep「超过上限」。
   回测往 2025-07-07 之前延伸要把门槛放回 4.8%,并先确认每天命中不超过 500。
   买入按 1 万 ÷ 收盘价取整股、**不按 100 股一手** —— 按手的话 100 元以上买不了或金额被抬高,每笔不等额(替用户做的决定,写在引擎文件头)。
   次日收盘跌停(收盘 = 最低且跌幅到门槛)或停牌视为卖不出,顺延。
   (f) **L-01 有涨幅上限(名义涨停 + 1.2 个百分点)**:全年核对发现 `screen_asof` 的拆股修正(扫描源 Perf 锚点)会把少数 A 股的
   前一根收盘改错 —— 600508 原始 9.43 → 8.86 跌 6%,修正后 7.42 → 8.86「涨 19.4%」被当涨停买入(300895 同理);反过来 300506 /
   301306 的真涨停被改没、600396 整只被丢掉。上限挡住假信号,漏掉的要修拆股修正本身(A 股日线本来就是腾讯前复权,修正可能根本不该做),没在这条线里修。
   **核对方法**:本地 4 天(2026-09-08 ~ 09-11)引擎买入 34 笔,与直接读 `rs_daily` 按同一规则算的信号逐笔一致;
   全年(加上限后)独立算法 3020 个信号、引擎 3015 笔全在其中;差的 5 笔:3 笔是上面的拆股修正,2 笔是新股上市头几天没有涨跌幅限制
   (688765 科创板 +24.28%、603165 主板 +11.44%)—— 独立算法没设上限才算进去,引擎不算涨停是对的;
   改规则后用同样的独立算法再对一次(脚本思路:按 code 分组取最近 5 根收盘,判三条)。
   (g) **v2 = v1 + L-07 5 日均线多头(2026-09-17 用户追加,口径用户选:当天收盘 > MA5 且 MA5 比前一天高)**。只改引擎,池子不变复用。
   v1 数据备份在 fin-r1 `~/agent-archive/2026-09-17_limitup-v1/`(成交 / 净值 / 持仓 / 元数据 CSV + sha256)。
   结果:完整交易 3003 → 2957,每笔净 -40 → -37 元,合计 -120,631 → -108,754 元,账户 -12.07% → -10.88%,回撤 -13.52% → -12.41%。
   **L-07 几乎是冗余条件**:涨停后三天收盘守在涨停价之上的票,绝大多数本来就站在向上的 5 日线上,只挡掉 46 笔(41 笔没站上 5 日线、5 笔 5 日线没向上);
   但挡掉的这 46 笔在 v1 里每笔 -258 元、合计 -11,877、赚钱 35%,方向是对的。独立算法(加 MA5)2972 个信号、引擎 2969 笔全在其中,差的 3 笔仍是拆股修正。
   (h) **v3 = v2 + L-08 只做创业板 / 科创板(2026-09-17 用户要求,开关 `growth_only`)**。v2 数据备份 `~/agent-archive/2026-09-17_limitup-v2/`。
   核对:v3 的 532 笔与 v2 里的双创 532 笔逐笔一致(键相同、每笔盈亏 0 差异),主板买入 0 笔。
   结果:每笔净 -37 → -22 元,合计 -108,754 → -11,559 元,账户 -10.88% → -1.16%(本金没变、笔数少了,账户百分比不可直接比),回撤 -2.24%。
   **扣费前合计 -3,485、手续费 8,074** —— 双创这组规则扣费前接近打平,亏损主要是手续费(每笔约 15 元);t -0.76 不显著。
   上下半年每笔 -23 / -21;创业板 364 笔每笔 -24、科创板 168 笔每笔 -17;13 个月里 6 个月为正;前 10 笔合计 +19,820,去掉后每笔 -60 —— 靠少数大赢家撑着。
   筛双创是看过 v1 分板块结果再挑的,换时间段验证之前别当结论。
   (i) **v4 = v3 + L-09 整理幅度 ≤ 15% + L-10 不许三天都缩量(2026-09-17)**。v3 备份 `~/agent-archive/2026-09-17_limitup-v3/`。
   用户原提「涨停日放量 或 三天缩量整理」,研究(双创 533 个信号 + 2025-06~09 检验段 181 个,脚本思路:原始 rs_daily 独立算信号与量价特征,按五分位 / 门槛网格看次日扣费后收益,
   分上下半年与检验段)结果相反:**涨停日放量越大越差**(量比 ≥ 3.6 每笔 -0.76%)、**三天每天都比涨停日缩量最差**(65 笔 -1.50%,t -2.36,三段都负)、
   三天均量 ≥ 涨停日 k 倍各档都不如 L-10;量比分母用 20 日还是 50 日均量几乎没区别(与次日收益相关 -0.056 / -0.046),**不单独加均量条件**。
   幅度 15% 是 13~18% 里的好点(14% -0.45%、16% -0.16%),不是稳定规律。用户看过数据后选了现口径。
   结果:149 笔(另 1 笔未平),每笔净 +50 元、合计 +7,388(扣费前 +9,655、手续费 2,267)、t 1.06,账户 +0.74%、回撤 -0.43%;
   上下半年每笔 +57 / +45;创业板 114 笔 +60、科创板 35 笔 +15;13 个月 8 个月为正;**前 10 笔 +14,385,去掉后每笔 -50**。
   核对:v4 149 笔全在 v3 里、每笔盈亏 0 差异;独立算法 151 个信号、引擎 150 笔全在其中,差的 1 笔是 300506 拆股修正。
   四轮条件都是在同一年数据上看结果加的,过拟合风险高;要当真得补更早的双创日线做样本外验证。
   (j) **新方向 `limitup_yin`「涨停三阴 · 主板」(2026-09-18 用户新开,`agent_limitup_yin.py` = `agent_limitup` 的 `mode="yin"`)**。
   规则 Y-01 主板涨停(同 L-01 的门槛与上限)· Y-02 之后三天每天**收盘 < 开盘**(严格,十字星不算;缺开盘价不买)· Y-03 只做主板(60 / 00 开头),
   买卖 / 仓位 / 手续费沿用 L-04 ~ L-06。**阴线按 K 线实体定义不是「收跌」,这是替用户定的口径**。原方向 limitup(v4)不动,研究台判定仍按 best = limitup。
   开盘价:引擎声明 `WITH_OPEN`,`Ctx` 给该市场装 6 元组(`screen_asof.bars_upto(with_open=True)`,第 6 个元素开盘价),其余引擎只按下标读前 5 个。
   回填必须 `research-backfill --line limitup --branch limitup_yin --from 2025-09-17`,**不带 --branch 会把 v4 方向重跑**。
   一年结果:1976 笔、每笔净 -48 元(中位 -75)、合计 -94,503(扣费前 -64,512、手续费 29,992)、t -4.87,账户 -9.45%、回撤 -10.82%;
   上下半年每笔 -58 / -41 都显著为负;沪市每笔 -67、深市 -30;13 个月 3 个月为正。核对:独立算法 1981 个信号与引擎 1981 笔逐笔一致。
   **部署注意(2026-09-18)**:fin-r1 当天被另一会话切成 GHCR 预构建镜像(`docker-compose.yml` 的 api 只有 `image:`,`build` 与 bind mount 挪到
   `docker-compose.dev.yml`)。`docker compose build api` 什么都不做,新代码要等发新版镜像才上线;回填用一次性容器挂仓库代码
   `-v ~/hunter-community/apps/api/app:/app/app`。在那之前线上看板 / 研究台看不到这个方向,每晚任务也不跑它。**2026-09-19 已随 sha-cceeb97 上线**,研究台接口里有 limitup_yin。
...
**研究台两列 + 规则冻结日(2026-09-18 用户定)**:看板只剩「运行中 / 封存」,淘汰的线仍不上看板;立项列去掉,「新建研究线」表单暂时保留
(提交后放运行中、标「待写引擎」)。状态值 idea / backtest / paper **不迁移**,只在 `agent_research.STATUS_TEXT` 里显示成「待写引擎 / 运行中」。
理由:全年回测和纸上跑是同一个引擎、同一份日线,每晚任务本来就让回测中的线往前跑,30 笔判定也数全段 —— 两列只是名字不同。
真正要区分的是**规则冻结之后的新数据**:每条线在 `LINES` 里记 `frozen_at`(最后一次改买卖规则的上海时间)与 `frozen_through`
(那一刻该市场日线的最新交易日;美股上海 06:30 入库、A 股 / 港股 17:30),研究台卡片单列「冻结后」一段,
**30 笔判定只数 entry_date > frozen_through 的完整交易**(`split_after` / `decide`,用例 `tests/test_agent_research.py`)。
**改任何一条研究线的买卖规则,必须在同一个提交里把这两个值改成改动那一刻**;只改文案、缓存、另开方向不算。
没写冻结日的新线过回测关那天自动以回测最后一天为冻结日。现值:突破买入 v14 · 09-15 13:25 / 09-14;涨停后强势整理 v4 · 09-17 23:50 / 09-17。

## 小鹿港股研究数据(2026-09-18 用户:「小鹿支持了美股和 A 股,现在要支持港股,准备所需的研究数据」)· 数据已备,研究线还没有

1. **日线已补到 2021-06-29**(`rs_daily` market='hk',基准恒指同起点),池 1650 只全有日线,共约 178 万行。原来只有 2025-05 起约 320 根,
   RS 要 253 根热身,等于没有回测窗口。**回溯扫描的精确 RS 从 2022-08-01 起可算**(排名池 1112 只,之前热身不够只有一百多只),
   可回测窗口约 4 年。做法:`rs_history.fetch_older` 三轮(end = 2025-05-27 / 2024-01-31 / 2022-10-13,每轮 = 基准最早日前一天)
   + 新进池的 228 只逐只拉最新窗口再往前翻到基准起点;一次性容器挂仓库代码、套 `flock /tmp/rs_history.lock`,1 次/秒,约 100 分钟,0 失败。
   **必须在 17:30(沪)A 股 / 港股每晚任务前跑完**,否则每晚任务 `flock -n` 拿不到锁直接跳过。
   接缝核对:三处接缝各随机 25 只,重拉一段跨接缝的窗口与库里逐日比,0 不一致(复权基准一致);接缝日平均绝对涨跌 1.8%(平时 2.2%)。
2. **港股池原来漏了 A+H 的 H 股和第二上市**(`4af678f`,已随 1.1.0-rc2 上线):扫描源把主上市地在别处的港股线标 `is_primary=False`,
   工行 / 比亚迪 / 宁德 / 平安 / 汇丰 / 阿里 / 京东 / 网易等 255 只一只都没有。港股改成 `currency = HKD`(剔 23 只人民币柜台)。
   快照 2398 → 2629、RS 排名池 1421 → 1650。副作用:补进来的有一批 2024~2025 年新上市的 A+H(宁德 3750、美的 0300 等)不足 253 根,
   精确法覆盖率 89.1% < 90%,线上港股 RS 整批退回快照插值法。**2026-09-18 用户拍板改精确法的分母**(`screen_rs.inject`):
   「扫描源也说是次新(SMA250 空)且我们也没有精确值」的票不计入;扫描源有 SMA250 却没精确值的(每晚没拉到)仍在分母里,门槛照拦。
   三个市场真实快照新旧对比:美股 / A 股评级 0 行不同(覆盖率 90.7% → 99.1% / 98.0% → 99.8%),港股快照法 → 精确法(98.2%)。
   **快照法那条路没改**(分母含次新股,用例「次新股太多时整批不给」),两条路口径不同是有意的。
   **上线状态(2026-09-18 用户定)**:bad0854 在 v1.1.0 之后,用户选「等下一个版本」,不为它把演示站从 1.1.0 改钉到 sha 镜像。
   **2026-09-19 12:40 已随 sha-cceeb97 上线**(用户要求立刻上线筛选器蓝线修复,演示站改钉 sha 镜像):线上 `plot scan = rs_rating >= 90;` 港股返回 `rs.method = exact`、命中 144 只。
   在那之前线上港股 RS 仍是快照插值法,属预期;发下一版后用 `plot scan = rs_rating >= 90;` 跑港股,看返回的 `rs.method` 是 exact 即生效。
3. **港股截头规则(`45617ab`,还不在 rc2 里)**:「最老锚点之前有单日 ±80% 跳变就截断」在港股截掉 246 只票的头部(12.3 万根),
   港股小盘单日翻倍很常见。逐次看 457 次跳变:374 次量没反向(真涨跌)、68 次量反向同倍(00064 价 ×19.6、量 ×0.046 = 20 合 1)——
   **腾讯港股前复权不可靠地处理合股**,规则不能删。港股改为传成交量、只在 `rs_history.split_like` 处截:单日 ≥5 倍一律截;
   涨时量缩得和价格倍数相当或更狠就截(单边 —— 仙股合股后流动性常塌得比倍数还狠,00627 价 ×91 量 ×0.004,第一版双边容差就漏了);
   跌时双边 1.6 倍。结果截头 136 只 / 5.8 万根。**美股 / A 股不传量,行为不变**(已有研究线回测依赖它)。
   另有 10 只锚点对不上又修不好整只不给数(00245 / 00497 / 00698 …,和补历史前一样)。
4. **已知偏差**:股票池 = 今天的排名池,**2021 年以来退市的港股不在里面**(幸存者偏差,港股仙股退市多,回测会偏乐观);
   开盘价缺 6002 行(0.3%);市值 / 财务没有历史值(回溯通用限制)。
5. **开第一条港股研究线时还要做的**(数据之外,都没做):
   (a) 引擎声明 `MARKET = "hk"`、`CURRENCY = "HKD"`、`BENCH_LABEL = "恒生指数"`、`INITIAL_CAPITAL`(`agent_opt._MARKET_LABEL` 已有 hk)。
   (b) `agent_run` 里按「us / 其它」二分的地方**港股会落进错的分支**:`_trade_rounds` 的手续费(非 a 走美股按股阶梯费!)、
   `fee_note` / `scope_note`(写的是 A 股)、`_trade_item` 成交时刻(A 股 15:00;港股 16:00 HKT)、`_next_run_text`(写的是 A 股)。
   (c) **手续费口径要问用户**(A 股线是立项时问的):港股印花税双边、交易征费、会财局征费、联交所交易费、结算费、佣金 —— 费率随年份变过,别凭记忆写。
   (d) **每手股数没有数据源**(港股每只不同,A 股线是替用户定了不按手);**财报日只有美股**(Nasdaq);港股无涨跌停、可 T+0,别照搬 A 股线的口径。
   (e) 每晚:crontab `30 9 * * *`(UTC,= 沪 17:30)已跑 `a hk` 两个市场的日线;`agent_run daily` 在美股之后跑,`run_latest` 会给其它市场各开一个 Ctx,港股线登记后自动跟上。
