...
2. **港股池原来漏了 A+H 的 H 股和第二上市**(`4af678f`,已随 1.1.0-rc2 上线):扫描源把主上市地在别处的港股线标 `is_primary=False`,
   ...
   快照 2398 → 2629、RS 排名池 1421 → 1650。**副作用:线上港股筛选器的 RS 退回快照插值法**(精确法覆盖率 89.1% < 90%)——
   补进来的有一批 2024~2025 年新上市的 A+H(宁德 3750、美的 0300 等),不足 253 根。快照路径分母含次新股是 `rs_ratings` 文档和用例
   「次新股太多时整批不给」写明的原项目口径,**没改,要改先问用户**。回溯 / 回测路径分母不含次新股(上面第 9 条),不受影响。
